...
public class Solution {
    ...
    private static final String LOAD_ALL_WITH_LIMIT_QUERY = "SELECT * FROM solution ORDER BY updated DESC LIMIT ?;";
    ...
    public static Solution[] loadAll(Connection con, int limit) throws SQLException {
        List<Solution> exerciseList = new ArrayList<Solution>();
        try ( PreparedStatement ps = con.prepareStatement(LOAD_ALL_WITH_LIMIT_QUERY) ) {
            ps.setInt(1, limit);
            try ( ResultSet rs = ps.executeQuery() ) {
                while ( rs.next() ) {
                    exerciseList.add(getFromResultSet(rs));
